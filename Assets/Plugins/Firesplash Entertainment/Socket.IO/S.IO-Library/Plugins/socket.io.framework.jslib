mergeInto(LibraryManager.library, {
	...
	CreateSIOInstance: function (instanceName, gameObjectName, targetAddress, enableReconnect, payload) {
		...
		var serverQuerySepIndex = fullAddress.indexOf('?');
		var serverAddress = (serverSepIndex > 0 ? fullAddress.substr(0, serverSepIndex) : (serverQuerySepIndex > 0 ? fullAddress.substr(0, serverQuerySepIndex) : fullAddress));
		var serverPathLength = (serverQuerySepIndex > 0 ? serverQuerySepIndex - serverSepIndex : fullAddress.length - serverSepIndex);
		var serverPath = (serverSepIndex > 0 ? fullAddress.substr(serverSepIndex, serverPathLength) : '/socket.io/');
		
		var queryParams = {};
		window.UnitySocketIOAuthPayloads[iName] = null;
		if (serverQuerySepIndex !== false) {
			var queryString = fullAddress.substring(serverQuerySepIndex + 1).split('&');
